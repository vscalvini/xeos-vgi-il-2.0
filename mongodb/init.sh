...
create_user="if (!db.getUser('user')) { db.createUser({ user: 'user', pwd: '$MONGODB_PASSWORD', roles: [ {role:'readWrite', db:'vgi_il'} ]}) }"
...
if test -n "$INIT_DUMP"; then
	...
	until mongo vgi_il $auth $INIT_DUMP; do sleep 5; done
fi
) &
